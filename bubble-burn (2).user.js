...
// @grant       GM_addStyle
// @connect     *
// @downloadURL https://github.com/hyun120207-alt/bubblechat-burner/raw/refs/heads/main/bubble-burn.user.js
// @updateURL   https://github.com/hyun120207-alt/bubblechat-burner/raw/refs/heads/main/bubble-burn.user.js
// ==/UserScript==
...
",
      ...
      i = `\n            <div id="chasm-burner" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.6); -webkit-backdrop-filter: blur(6px); backdrop-filter:blur(6px); z-index: 9999; display: flex; justify-content: center; align-items: center;">
                <div id="cb-content" style="background: ${o.modalBg}; color: ${o.textColor}; padding: 20px; border-radius: 8px; width: ${l}px; min-height: 500px; display: flex; flex-direction: column;">
                    <style>
                        .cb-spinner {
                            display: inline-block;
                            width: 16px;
                            height: 16px;
                            border: 2px solid ${o.buttonText};
                            border-radius: 50%;
                    ...
                    <div id="cb-tab-content" style="flex-grow: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; padding: 10px 0;">
                        ...
                        <div id="cb-burner-content" style="display: block;">
                            <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                                <div style="flex: 1;">
                                    <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">불러올 메시지 수 (최대 50)</label>
...
<option value="custom">직접 입력</option></optgroup>
                            ...
                            <div id="cb-openrouter-model-container" style="display: ${'openrouter' === t.provider ? "block" : "none"};">
                                <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">
                                    OpenRouter 모델</label>
                                <select id="cb-openrouter-model-select" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 10px; background: ${o.modalBg}; color: ${o.textColor};">
                                ...
                                <input id="cb-openrouter-model-custom" type="text" placeholder="커스텀 OpenRouter 모델 입력" value="${t.openRouterModel || ""}" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor}; display: none;">
                            </div>
       <div style = "display: flex; flex-direction: row;">                      <label style="display: flex; align-items: center; gap: 5px; font-size: 0.9em; color: ${o.textColor}; margin-bottom: 15px;">
                                <input id="cb-attach-usernote" type="checkbox" style="background: ${o.modalBg}; color: ${o.textColor};">
                                <span>유저노트 첨부</span>
                            </label><label style="display: flex; align-items: center; gap: 5px; font-size: 0.9em; margin-left: 8px; color: ${o.textColor}; margin-bottom: 15px;">
                                <input id="cb-add-header" type="checkbox" style="background: ${o.modalBg}; color: ${o.textColor};">
                                <span>무작위 헤더 추가</span>
                            </label></div><div style = "display: flex; flex-direction: row;">
      <label style="display: flex; align-items: center; gap: 5px; font-size: 0.9em; color: ${o.textColor}; margin-bottom: 15px;">
                                <input id="cb-auto-retry" type="checkbox" style="background: ${o.modalBg}; color: ${o.textColor};">
                                <span>서버 오류 발생시 자동 재시도</span>
                            </label>  <label style="display: flex; align-items: center; gap: 5px; font-size: 0.9em; margin-left: 8px; color: ${o.textColor}; margin-bottom: 15px;">
                                <input id="cb-use-vertex-ai" type="checkbox" style="background: ${o.modalBg}; color: ${o.textColor};">
                                <span>Gemini Vertex AI 사용</span>
                            </label></div>
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                                <label style="font-size: 0.9em; color: ${o.textColor};">실행 로그</label>
                                <div style = "display: flex; flex-direction: row;"><div class = "hourglass-container" style="margin-right: 10px;"> ${HOURGLASS_SVG} </div><div id="cb_timer" style="font-size: 0.9em; color: ${o.textColor};">00:00</div>
                            </div></div>
                            <textarea id="cb-log" readonly style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; height: 100px; resize: none; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor};"></textarea>
                        ...
                        <div id="cb-settings-content" style="display: none;">
                            ...
                            <div id="cb-gemini-api-container" style="display: ${'gemini' === t.provider ? "block" : "none"};">
                                <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">
                                    Gemini API 키 <a href="https://aistudio.google.com/app/apikey" target="_blank" style="color: ${o.buttonBg}; text-decoration: none;">(발급받기)</a>
                                    <a href="https://ai.google.dev/gemini-api/docs/rate-limits?hl=ko" target="_blank" style="color: ${o.buttonBg}; text-decoration: none;">(무료 모델 및 제한 확인)</a>
                                </label>
                                <input id="cb-gemini-api-key" type="text" placeholder="Gemini API 키" value="${t.geminiKey || ""}" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor};">
                            </div>
                         <div id="cb-vertex-ai-api-container" style="display: ${'vertexai' === t.provider ? "block" : "none"};">
                                <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">
                                    Vertex AI 설정 <a href="https://discord.com/channels/1372949645436915844/1410482670001066126" target="_blank" style="color: ${o.buttonBg}; text-decoration: none;">(API 스크립트 발급 방법 [디스코드])</a>
                                </label>
                            ...
                            </div>   <div id="cb-openrouter-api-container" style="display: ${'openrouter' === t.provider ? "block" : "none"};">
                                <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">
                                    OpenRouter API 키 <a href="https://openrouter.ai/settings/keys" target="_blank" style="color: ${o.buttonBg}; text-decoration: none;">(발급받기)</a>
                                    <a href="https://openrouter.ai/models?context=64000&max_price=0&order=context-high-to-low" target="_blank" style="color: ${o.buttonBg}; text-decoration: none;">(무료 모델 목록 확인)</a>
                                </label>
                                <input id="cb-openrouter-api-key" type="text" placeholder="OpenRouter API 키" value="${t.openRouterKey || ""}" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor}; display: none;">
                            </div>
                            <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                                <a href="https://discord.com/channels/1372949645436915844/1372949645990432911" target="_blank" style="font-size: 0.9em; color: ${o.buttonText}; text-decoration: none; padding: 5px 10px; background: ${o.buttonBg}; border-radius: 4px;">문의 및 건의</a>
                                <a href="https://discord.com/invite/hEb44bUFgu" target="_blank" style="font-size: 0.9em; color: ${o.buttonText}; text-decoration: none; padding: 5px 10px; background: ${o.buttonBg}; border-radius: 4px;">사용법</a>
                                <a href="https://archive.is/GwboB" target="_blank" style="font-size: 0.9em; color: ${o.buttonText}; text-decoration: none; padding: 5px 10px; background: ${o.buttonBg}; border-radius: 4px;">자주 묻는 질문</a>
                            </div>
                            <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px; cursor: pointer;" id="cb-advanced-toggle">고급 사용자 설정 ▼</label>
                            <div id="cb-advanced-content" style="display: none; margin-bottom: 15px;">
                                <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">추가 프롬프트 CDN 주소 (엔터로 구분)</label>
                                <textarea id="cb-cdn-list" placeholder="추가 CDN URL 목록" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; height: 100px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor};">${t.cdnUrls.join(
        "\n"
      )}</textarea>
                            </div>
                            <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">전송 시 사용자 메시지</label>
                            <input id="cb-user-message" type="text" placeholder="전송 메시지" value="${t.userMessage}" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor};">
                            <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">요약본 최상단 텍스트</label>
                            <input id="cb-prepend-text" type="text" placeholder="요약 앞에 추가 (빈 값: 사용 안함)" value="${t.prependText}" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor};">
                            <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">요약본 최하단 텍스트</label>
                            <input id="cb-append-text" type="text" placeholder="요약 뒤에 추가 (빈 값: 사용 안함)" value="${t.appendText}" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor};"><button id="cb-save-settings" style="width: 100%; padding: 10px 20px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer;">저장</button>
                        </div>
                    </div>
                </div>
            </div>
      ...
      A.addEventListener("click", async () => {
        ...
        try {
          ...
          if (location.hostname.includes("bubblechat.ai")) {
            ...
            T.value = `[${p()}] 총 ${k}턴 (${2 * k} 메시지) ${messages.length}개 가져옴\n${T.value}`;
          }
          ...
          (T.value = `[${p()}] ${'vertexai' === $ ? "Firebase Vertex AI " : $.charAt(0).toUpperCase() + $.slice(1)} 요청 시작 (총 ${N.length}자 요청, 요청 모델: ${'gemini' === $ || 'vertexai' === $ ? t.geminiModel : t.openRouterModel})
${T.value}`),
            (r = setInterval(() => {
              const n = Math.floor((Date.now() - s) / 1e3);
                ...
                try {
                  ...
                  while (
                    ...
                    if (geminiResponse.status === 429) {
                      ...
                      T.value = `[${p()}] 서버 오류로 Gemini API 요청이 실패하였습니다 - 잠시 후 재시도합니다. (${result?.error?.message ?? "알 수 없음"}) \n${T.value}`;
                  ...
                  if (!geminiResponse.ok) {
                    if (geminiResponse.status === 403) {
                      alert("Gemini API키가 올바르지 않습니다.\nAPI 키를 확인하고 다시 시도해주세요.");
                    } else if (geminiResponse.status === 429) {
                      alert("Gemini API의 레이트리밋에 도달하여 요청에 실패하였습니다.\n잠시 후 다시 시도하거나, 자동 재시도 옵션을 사용하세요.");
                    } else if (geminiResponse.status === 500) {
                      alert("Gemini API에서 서버 오류가 발생하였습니다.\n잠시 후 다시 시도하거나, 자동 재시도 옵션을 사용하세요.");
                    } else if (geminiResponse.status === 503) {
                      alert("Gemini API의 레이트리밋에 도달하여 요청에 실패하였습니다.\n잠시 후 다시 시도하거나, 자동 재시도 옵션을 사용하세요.");
                    } else {
                      throwError(
          ...
          b(),
            (T.value = `[${p()}] ${$.charAt(0).toUpperCase() + $.slice(1)} 요청 완료 (소요 시간: ${Math.floor((Date.now() - s) / 1e3)}초)\n${T.value}`),
            j
              ? (function (a, r, l) {
                  n++;
                  const i = `cb-result-${n}`,
                  ...
                  (d.id = i),
                    ...
                    (d.innerHTML = `\n                <textarea id="${i}-text" style="width: 100%; padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; height: 300px; resize: vertical; margin-bottom: 15px; background: ${o.modalBg}; color: ${o.textColor}; display: block;">${a}</textarea>\n                <div id="${i}-count" style="font-size: 0.9em; color: ${o.textColor}; margin-bottom: 4px; display: block;">글자 수: ${a.length} (최대 5000자, ${Math.ceil(a.length / e)}개 메시지 분할 전송)</div>\n                <div id="${i}-model-info" style="font-size: 0.8em; color: ${o.textColor}; margin-bottom: 16px; display: block;">[ ${l === "vertexai" ? "Google Vertex AI" : l.charAt(0).toUpperCase() + l.slice(1)} ] — ${r}</div>\n         \n                    <div id = "chasm-status-${n}" class = "chasm-burner-status"> 
                    ...
                    <button id="${i}-send" style="padding: 10px 20px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer; display: block;">전송</button>
                    <button id ="sandbox-html-${n}" class = "html-display-button" disabled="true" content-reference="${i}-text"> <span> HTML 표시 </span> </button>  
                                      <div id="${i}-status" style="font-size: 0.9em; color: ${o.textColor}; margin-left: 10px; display: none;"></div>
                </div>
            `),
                  ...
                  p.addEventListener(
                    "input",
                    c(() => {
                      m.textContent = `글자 수: ${p.value.length} (최대 5000자, ${Math.ceil(p.value.length / e)}개 메시지 분할 전송)`;
                    }, 200)
                  );
                  if (isCharacterPath()) {
                  ...
                  document
                    ...
                    .addEventListener("click", async () => {
                      if (isCharacterPath()) {
                        alert("결정화 캐즘의 현재 버전에서는 캐릭터로의 전송을 지원하지 않습니다.");
                        return;
                      }
                      const n = document.getElementById(`${i}-status`);
                      n.style.display = "inline";
                      const o = (e, t = !1) => {
                        n.innerHTML = `${e}${t ? '<span class="cb-spinner"></span>' : ""}`;
                      };
                      o("유저 입력 보내는 중", true);
                      let a = p.value;
                      const r = new y(),
                      ...
                      confirm("전송이 완료되었습니다! 페이지를 새로고침하시겠습니까?")
                        ? location.reload()
                        : (n.style.display = "none");
                    });
                  G(s, d);
                })(j, q, $)
              : ((T.value = `[${p()}] ${$.charAt(0).toUpperCase() + $.slice(1)} API 응답: ${JSON.stringify(j, null, 2)}
${T.value}`),
                alert(
                  `${$.charAt(0).toUpperCase() + $.slice(1)} API 처리에 실패했습니다.`
                ));
        } catch (n) {
      ...
      // --- Start of Backup/Restore Injection ---
      function setupBackupRestore(o) {
        const backupTab = document.getElementById("cb-tab-backup");
        if (!backupTab) return; // In case the button wasn't added

        const backupContent = document.createElement('div');
        backupContent.id = 'cb-backup-content';
        backupContent.style.display = 'none';
        backupContent.innerHTML = `
            <div style="padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px; margin-bottom: 20px;">
                <h3 style="margin-top: 0; margin-bottom: 10px;">백업 (Backup)</h3>
                <p style="font-size: 0.9em; margin-bottom: 15px;">현재 채팅의 모든 메시지를 JSON 파일로 다운로드합니다. 이 파일은 복원 기능에 사용할 수 있습니다.</p>
                <button id="cb-backup-button" style="width: 100%; padding: 10px 20px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer;">대화 내용 백업 시작</button>
            </div>
            <div style="padding: 10px; border: 1px solid ${o.borderColor}; border-radius: 4px;">
                <h3 style="margin-top: 0; margin-bottom: 10px;">복원 (Restore)</h3>
                <p style="font-size: 0.9em; margin-bottom: 15px;">백업된 JSON 파일을 선택하면 모든 대화 내용이 현재 채팅 입력창에 붙여넣기됩니다. 그 후 직접 전송 버튼을 눌러주세요.</p>
                <input type="file" id="cb-restore-input" accept=".json" style="width: 100%; padding: 8px; border: 1px solid ${o.selectBorder}; border-radius: 4px; background: ${o.selectBg}; color: ${o.selectText}; margin-bottom: 10px;">
                <div id="cb-restore-status" style="font-size: 0.9em; margin-top: 10px; color: ${o.textColor};"></div>
            </div>
        `;
        document.getElementById('cb-tab-content').appendChild(backupContent);

        backupTab.addEventListener('click', () => G(backupTab, backupContent));

        document.getElementById('cb-backup-button').addEventListener('click', () => {
            try {
                const getMessagesFromDOMforBackup = () => {
                  const messages = [];
                  const messageNodes = document.querySelectorAll('div[class*="MessageItem__MessageContainer"]');
                  messageNodes.forEach(node => {
                      const userNode = node.querySelector('div[class*="UserMessage__SContainer"] div[class*="MarkdownRenderer__SWrapper"]');
                      const charNode = node.querySelector('div[class*="CharacterMessage__SContainer"] div[class*="MarkdownRenderer__SWrapper"]');
                      if (userNode) {
                          messages.push({ role: 'user', content: userNode.innerText.trim() });
                      } else if (charNode) {
                          messages.push({ role: 'assistant', content: charNode.innerText.trim() });
                      }
                  });
                  return messages.filter(m => m.content);
                };

                const messagesToBackup = getMessagesFromDOMforBackup();
                if (messagesToBackup.length === 0) {
                    alert("백업할 메시지를 찾을 수 없습니다. 채팅이 화면에 표시되고 있는지 확인하세요.");
                    return;
                }
                
                const backupData = {
                    exportedAt: new Date().toISOString(),
                    source: "bubble-burn-backup",
                    profile: "User",
                ...
                a.download = `bubble-chat-backup-${timestamp}.json`;
        ...
        document.getElementById('cb-restore-input').addEventListener('change', (event) => {
            const file = event.target.files[0];
            if (!file) return;
            const reader = new FileReader();
            const statusDiv = document.getElementById('cb-restore-status');
            statusDiv.textContent = '파일을 읽는 중...';
            reader.onload = (e) => {
                try {
                    const data = JSON.parse(e.target.result);
                    if (!data.messages || !Array.isArray(data.messages)) throw new Error("Invalid backup file.");

                    const findInput = () => {
                        const candidates = ['div[class^="ChatInput__STextarea"] textarea', "form textarea", "textarea", 'div[contenteditable="true"]'];
                        for (const sel of candidates) {
                            const el = document.querySelector(sel);
                            if (el) return el;
                        }
                    ...
                    const input = findInput();
                    if (input) {
                        const fullLog = data.messages.map(m => `[${m.role === 'user' ? 'USER' : 'CHARACTER'}]: ${m.content}`).join('\n\n---\n\n');
                        setValueAndNotify(input, fullLog);
                        statusDiv.textContent = `${data.messages.length}개의 메시지를 복원하여 입력창에 붙여넣었습니다.`;
                    } else {
      ...
      setupBackupRestore(o);
      // --- End of Backup/Restore Injection ---
