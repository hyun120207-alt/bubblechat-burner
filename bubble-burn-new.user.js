...
!(async function () {
  ...
  function throwError(
    ...
    throw (
      (prompt(
        "구동 중 예상치 못한 오류가 발생하였습니다. 다음 내용을 복사하여 제미나이에 물어보거나' 카카오톡 오픈채팅으로 문의해주세요.\n\n",
  ...
  async function C() {
    ...
    const t = $.getConfig(),
      ...
      <label style="display: flex; align-items: center; gap: 5px; font-size: 0.9em; color: ${
        ...
        o.textColor
                            ...
                            <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">채팅 백업/복원</label>
                            <div style="display: flex; gap: 5px; margin-bottom: 10px;">
                                <button id="cb-backup-chat" style="flex: 1; padding: 10px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer;">채팅 백업</button>
                                <button id="cb-restore-chat" style="flex: 1; padding: 10px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer;">채팅 복원</button>
                            </div>
                            <input type="file" id="cb-file-chat" style="display: none;" accept=".json">

                            <label style="font-size: 0.9em; color: ${o.textColor}; display: block; margin-bottom: 5px;">설정 백업/복원</label>
                            <div style="display: flex; gap: 5px; margin-bottom: 15px;">
                                <button id="cb-backup-settings" style="flex: 1; padding: 10px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer;">설정 백업</button>
                                <button id="cb-restore-settings" style="flex: 1; padding: 10px; background: ${o.buttonBg}; color: ${o.buttonText}; border: none; border-radius: 4px; cursor: pointer;">설정 복원</button>
                            </div>
                            <input type="file" id="cb-file-settings" style="display: none;" accept=".json">
    ...
    (X.style.display = "custom" === v.value ? "block" : "none"),
      ...
      A.addEventListener("click", async () => {
        ...
        try {
          ...
          if (location.hostname.includes("bubblechat.ai")) {
            try {
              const getMessagesFromDOM = () => {
                const extracted = [];
                // Select all potential message wrappers in order of appearance
                // CharacterBubble__SCharacterWrap: User and Character messages
                // NarrativeBubble__SWrapper: System/Narrative messages
                const wrappers = document.querySelectorAll('div[class*="CharacterBubble__SCharacterWrap"], div[class*="NarrativeBubble__SWrapper"]');
                
                if (wrappers.length === 0) {
                    // Fallback: Try older selectors if the new ones fail
                    const candidateSelectors = [
                        '[class*="Message__SContainer"]', 
                        '.character-text',
                    ...
                    for (const sel of candidateSelectors) {
                        ...
                        if (nodes.length > 0) {
                            console.log(`[C2-Burner+] Fallback: Found nodes with selector: ${sel}`);
                            return [...nodes].map(n => ({ 
                                content: n.innerText.trim(), 
                                role: "user", // Fallback assumption
                                name: "Unknown" 
                            }));
                        }
                    }
                ...
                wrappers.forEach(wrapper => {
                    ...
                    if (className.includes('NarrativeBubble__SWrapper')) {
                        const textNode = wrapper.querySelector('[class*="NarrativeBubble__SCenterBubble"]');
                        if (textNode) {
                            extracted.push({
                                role: "system",
                    ...
                    // 2. Character / User Message
                    // 'right' class indicates User, 'left' (or absence) indicates Character
                    const isUser = className.includes('right');
                    const role = isUser ? "user" : "model";
                    
                    // Attempt to find name
                    // Name is usually in TextWithChips__SWrapper inside the wrapper
                    const nameNode = wrapper.querySelector('[class*="TextWithChips__SWrapper"]');
                    const name = nameNode ? nameNode.innerText.trim() : (isUser ? "User" : "Character");

                    // Find all text bubbles within this wrapper
                    // They are usually in CharacterBubble__SCharacterText
                    const textNodes = wrapper.querySelectorAll('[class*="CharacterBubble__SCharacterText"]');
              ...
              T.value = `[${p()}] bubblechat.ai DOM에서 ${messages.length}개의 메시지를 추출했습니다 (User/Model/System 구분됨).\n${T.value}`;
            } catch (err) {
          ...
          const P = messages.map((n) => ({
            message: n.content || n.message, // Support both 'content' and 'message' keys
            role: n.role,
            username: n.name || ("user" === n.role ? O.name : undefined),
          }));
          ...
          const chatBackupBtn = document.getElementById("cb-backup-chat");
      const chatRestoreBtn = document.getElementById("cb-restore-chat");
      const chatFileInput = document.getElementById("cb-file-chat");

      chatBackupBtn.addEventListener("click", () => {
        const messages = getMessagesFromDOMForBackup();
        if (messages.length === 0) {
          alert("백업할 메시지가 없습니다.");
          return;
        }
        ...
        a.href = url;
        a.download = `bubble-chat-backup-${new Date().toISOString()}.json`;
        ...
        reader.onload = (e) => {
          try {
            ...
            if (backupData && backupData.messages) {
              restoreMessagesToDOM(backupData.messages);
              alert(`${backupData.messages.length}개의 메시지를 복원했습니다.`);
            } else {
              alert("잘못된 백업 파일 형식입니다 (메시지 데이터 없음).");
            }
          } catch (error) {
      ...
      const settingsBackupBtn = document.getElementById("cb-backup-settings");
      const settingsRestoreBtn = document.getElementById("cb-restore-settings");
      const settingsFileInput = document.getElementById("cb-file-settings");
      ...
      settingsBackupBtn.addEventListener("click", () => {
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `chasm-settings-backup-${new Date().toISOString()}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        ...
        reader.onload = (e) => {
          try {
            ...
            if (typeof config === 'object' && config !== null) {
                if(confirm("설정을 복원하시겠습니까? 현재 설정이 덮어씌워집니다.")) {
                    $.setConfig(config);
                    alert("설정이 복원되었습니다. 적용을 위해 페이지를 새로고침합니다.");
                    location.reload();
                }
            } else {
      ...
      function getMessagesFromDOMForBackup() {
        const messages = [];
        console.log("Backup: Starting extraction with Burner-matched logic...");

        // 1. Try the robust wrapper selector (same as Burner)
        // CharacterBubble__SCharacterWrap: User and Character messages
        // NarrativeBubble__SWrapper: System/Narrative messages
        const wrappers = document.querySelectorAll('div[class*="CharacterBubble__SCharacterWrap"], div[class*="NarrativeBubble__SWrapper"]');
        ...
        if (wrappers.length > 0) {
            wrappers.forEach(wrapper => {
                ...
                // Case A: Narrative / System Message
                if (className.includes('NarrativeBubble__SWrapper')) {
                    const textNode = wrapper.querySelector('[class*="NarrativeBubble__SCenterBubble"]');
                    if (textNode) {
                        messages.push({
                            author: "System",
                            content: textNode.innerHTML, // Keep HTML for backup/restore
                            isUser: false,
                            role: "system"
                        });
                    }
                ...
                // Find all text bubbles within this wrapper
                // Usually in CharacterBubble__SCharacterText
                const textNodes = wrapper.querySelectorAll('[class*="CharacterBubble__SCharacterText"]');
                textNodes.forEach(node => {
                        messages.push({
                        author: name,
                        content: node.innerHTML, // Keep HTML
                        isUser: isUser,
        ...
        // 2. Fallback if the primary logic found nothing (e.g. different site structure or failure)
        if (messages.length === 0) {
            console.warn("Backup: No messages found with primary logic. Trying fallback selectors.");
            
            const containerSelectors = [
                'div[class*="MessageItem__MessageContainer"]',
                'div[class*="Message__SContainer"]',
                'div[class*="ChatBubble__Bubble"]',
                '.message-item'
            ];
            ...
            messageNodes.forEach(node => {
                const authorSelectors = ['div[class*="MessageItem__Author"]', '[class*="AuthorName"]', '.author', 'strong'];
                const contentSelectors = ['div[class*="MarkdownRenderer__SWrapper"]', '[class*="BubbleContent"]', '.markdown-body', '.content'];
                
                let authorNode = null;
                for (const sel of authorSelectors) {
                    authorNode = node.querySelector(sel);
                    if (authorNode) break;
                }

                let contentNode = null;
                for (const sel of contentSelectors) {
                    contentNode = node.querySelector(sel);
                    if (contentNode) break;
                }

                const parentClass = node.parentElement ? node.parentElement.className.toLowerCase() : "";
                const isUserMessage = parentClass.includes('user') || 
                                      parentClass.includes('me') || 
                                      parentClass.includes('right');

                if (contentNode) {
                    messages.push({
                        author: authorNode ? authorNode.innerText.trim() : (isUserMessage ? "User" : "Character"),
        ...
        console.log(`Backup: Extracted ${messages.length} messages.`);
        return messages;
      }

      function restoreMessagesToDOM(messages) {
        const containerSelectors = [
            'div[class*="ChatListContainer__SListContainer"]',
            '[class*="ChatList"]',
            'main',
            '#root' 
        ];
        ...
        messages.forEach(msg => {
            // Create a wrapper that mimics the structure slightly for visual separation
            const messageWrapper = document.createElement('div');
            messageWrapper.style.cssText = "display: flex; flex-direction: column; margin-bottom: 12px; padding: 8px; border: 1px dashed #555; border-radius: 8px; background: rgba(0,0,0,0.1);";
            messageWrapper.className = 'restored-message-item';

            const metaHeader = document.createElement('div');
            metaHeader.style.cssText = "display: flex; justify-content: space-between; font-size: 0.8em; color: #888; margin-bottom: 4px;";
            metaHeader.innerHTML = `<span>${msg.author || (msg.isUser ? "User" : "Character")}</span><span>${msg.isUser ? "User" : "Character"}</span>`;

            const contentElement = document.createElement('div');
            contentElement.style.cssText = "font-size: 1em; line-height: 1.5;";
            contentElement.innerHTML = msg.content;

            messageWrapper.appendChild(metaHeader);
